...
const OfferInList = ({ offer, isstatus, fetchOfertas }) => {
  ...
  const askBeforeDeleting = (offer_id) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Sí",
          onPress: async () => await deleteOffer(offer_id),
        },
      ]
    );
  };
  ...
  const deleteOffer = async (offer_id) => {
    ...
    const res = await fetch(
      BASE_URL + "delete-offer/" + restaurantChosen.pk + "/",
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ offer: offer_id }),
      }
    );

    var jsonData2 = await res.json();
    if (jsonData2.status == 'ok'){
    fetchOfertas(restaurantChosen.pk)
    }
  };
  ...
  const navigateAndTrack = async (restaurantChosen_pk, id) => {
    navigation.navigate("Oferta/Promoción", { eventId: id });
  };
  ...
  return (
    <View style={offer.visible? styles.greencard : styles.redcard}>
      ...
      <View style={styles.insidecard}>
        ...
        >
          <Text style={styles.textbutton}>Ver oferta</Text>
        </TouchableOpacity>
        <TouchableOpacity
...
export default memo(OfferInList);
