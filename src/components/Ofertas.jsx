...
const Ofertas = ({onlylastweek=false, route}) => {
    ...
    const fetchOfertas = async (restaurantChosen_pk) => {
        const res2 = await fetch(BASE_URL + 'ofertas-digital-chef/' + restaurantChosen_pk + '/', {
            method: "GET",
            mode: "no-cors",
            headers: {
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener("focus", async () => {
          ...
          fetchOfertas(restaurantChosen_pk);
        }
        });
        return unsubscribe;
      }, [navigation]);
  ...
  useEffect(() => {
    ...
    return (
        ...
        {WARNING_NOT_SCROLLABLE ? <Text style={styles.textsmall}>Estás viendo el feed de ofertas del restaurante {restaurantChosen.franchise} localizado en {restaurantChosen.address}</Text>: null}
    
            <ScrollView>
            {loading && !gotten ? <ActivityIndicator size={33} />: !loading && !gotten? <Text style={styles.textsmall}>No puedes acceder a estos datos</Text>:<OffersList onlylastweek={onlylastweek} ofertas={[...ofertas]} fetchOfertas={fetchOfertas}/>}
            </ScrollView>
                  <TouchableOpacity
                      style={styles.reservation}
                      onPress={() => {
                        navigation.navigate("Creación de oferta");
                      }}
                    >
                      <Text style={styles.reservationtext}>Crear oferta</Text>
                    </TouchableOpacity>
         </View>
    )
}
