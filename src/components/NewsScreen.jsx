...
const NewsScreen = (variable) => {
  ...
  const updateNew = async (restaurantChosen_pk) => {
    ...
    if (!title || !description || !readableDate || !publishingDateString ){
      setDisabled(false)
    ...
    data.append("pk_of_new", variable.route.params.eventId.toString());
    data.append("title", title);
    data.append("readableDate", readableDate);
    data.append("publishingdate", publishingDateString);
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false)
      Alert.alert("Éxito", "Se ha actualizado la noticia");
      navigation.navigate('Novedades', { refresh: true })
    }
  };
  ...
  const fetchNews = async (restaurantChosen_pk) => {
    ...
    setDate(item.publishingdate);
    setReadableDate(item.readabledate);
    setPublishingDateString(item.publishingdate);
    setImage(item.image);
          ...
          <View style={styles.viewable}>
            ...
            <View>
              ...
              >
                <TextInput
                  style={styles.textinput}
                  placeholder="El día que quieres que aparezca esta novedad"
