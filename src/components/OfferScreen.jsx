...
const OfferScreen = (variable) => {
  ...
  const [pkOfProduct, setPkOfProduct] = useState(null)
  ...
  const updateOffer = async (restaurantChosen_pk) => {

    setDisabled(true)

    if (!title || !description || !readableDate || !publishingDateString || !publishingDate2 ){
      setDisabled(false)
      Alert.alert('¡Tienes que rellenar todos los campos!','(Puedes dejar el código vacío, así como la media, el descuento y los productos asociados)')
      return
    }
    let data = new FormData();
    data.append("pk_of_offer", variable.route.params.eventId.toString());
    ...
    data.append('pkOfProduct', pkOfProduct)
    data.append('discount_by_code_char', discountPercentage)
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false)
      Alert.alert("Éxito", "Se ha actualizado la oferta");
      navigation.navigate('Ofertas y Promociones', { refresh: true })
    } else if (jsonData4.hasOwnProperty('non_field_errors')){
      setDisabled(false);
      Alert.alert('Ha habido un problema', jsonData4.non_field_errors[0])
    }
  };
  ...
  const fetchNews = async (restaurantChosen_pk) => {
    ...
    setImage(item.image);
    if (item.dish_associated && item.discount_by_code){
      setPkOfProduct(item.dish_associated.join(","))
      setDiscountPercentage(item.discount_by_code.toString())
      }
      ...
      {loading ? (
        ...
        <ScrollView>
          <View style={styles.viewable}>
            ...
            <View>
              <Text style={styles.textBoldMarginTop}>Código</Text>
              <TextInput
                style={styles.textinput}
            ...
            <View>
            <Text style={styles.text}>PK del producto asociado a esta oferta</Text>
              <TextInput
                keyboardType='numeric'
                style={styles.textinput}
                onChangeText={setPkOfProduct}
                value={pkOfProduct}
              ></TextInput>
            </View>
            <View>
            <Text style={styles.text}>Porcentaje de descuento al producto asociado</Text>
              <TextInput
                keyboardType='numeric'
                style={styles.textinput}
            ...
            activeOpacity={disabled ? 1 : 0.7} 
              ...
              onPress={() => {
                updateOffer(restaurantChosen.pk);
              }}
            >
