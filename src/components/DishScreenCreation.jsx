...
const DishScreen = (variable) => {
  ...
  const renderCategoryList = () => {
    if (categoryState != []) {
      return categoryState.map((categoryElement, index) => {
        return (
          <Picker.Item
            label={categoryElement.menu_category}
  ...
  const updateNew = async (restaurantChosen_pk) => {
    ...
    const res4 = await fetch(
      BASE_URL + "create-dish-digital-chef/" + restaurantChosen_pk + "/",
      {
        method: "POST",
        headers: {
    ...
    if (jsonData4.status == "ok") {
      ...
      setDisabled(false)
      Alert.alert('Problema', jsonData4.message)
    }
  };

  const fetchDish = async (restaurantChosen_pk) => {
    const res4 = await fetch(
      BASE_URL + "get-only-menu-categories/" + restaurantChosen_pk + "/",
      {
        method: "GET",
        mode: "no-cors",
        headers: {
    ...
    if (res.status == 'unauthorized'){
      Alert.alert('Problema','Necesitas ser propietario o manager para cambiar las características de los platos')
      //setLoading(false)
      return  
    }
