...
const SorteosScreen = (variable) => {
  ...
  const createSorteo = async (restaurantChosen_pk) => {
    setDisabled(true)
    ...
    const res4 = await fetch(
      BASE_URL + "crear-sorteo-digital-chef/" + restaurantChosen_pk + "/",
      {
        method: "POST",
        headers: {
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false)
      Alert.alert("Éxito", "Se ha creado el sorteo");
      navigation.goBack();
      ...
      <ScrollView>
        <View style={styles.viewable}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              createSorteo(restaurantChosen.pk);
            }}
          >
