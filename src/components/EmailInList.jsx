...
const EmailInList = ({ email }) => {
  ...
  const iniciarCampaña = async () => {
    ...
    const res = await fetch(BASE_URL + "iniciar-campaña-email/" + restaurant_pk + '/' + email.id + '/', {
      method: "POST",
      mode: "no-cors",
      headers: {
    ...
    <>
      <View>
        <View style={styles.card}>
          ...
          <BouncyCheckbox
            size={25}
            isChecked={email.added_br}
            fillColor="black"
            unFillColor="#FFFFFF"
            useBuiltInState={false}
            text="Añadidos los br"
          ...
          {!started? <TouchableOpacity
                  onPress={() => {
                    navigation.navigate('Edición de email', { eventId: email.id, refresh: true })
                  }}
                  style={styles.buttongreen}
                >
                  <Text style={styles.textbutton}>Editar Email</Text>
          </TouchableOpacity>
          :<Text style={styles.textstarted}>El envío de este email ya se ha iniciado, así que no puedes editar el email</Text>}

          <TouchableOpacity
                  onPress={() => {
                    navigation.navigate('Emails de la campaña', { eventId: email.id, refresh: true })
                  }}
                  style={styles.buttonyellow}
                >
                  <Text style={styles.textbuttonblack}>Emails de la campaña</Text>
          </TouchableOpacity>
        </View>
      </View>
    </>
...
export default memo(EmailInList);
