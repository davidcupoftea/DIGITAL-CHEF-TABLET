      ...
      <View style={styles.card}>
        <View style={styles.insidecard}>
          ...
          <View>
            <Text style={styles.text}>
              Restaurante:{"\n"}
              {factura.restaurant.franchise}, en {factura.restaurant.address},{" "}
              {factura.restaurant.localidad}
            </Text>
          </View>
          <View>
            <Text style={styles.text}>
              Tipo de factura:{"\n"}
              {factura.ticket_identifier_serie}
            </Text>
          </View>
          <View>
            <Text
              style={factura.negative ? styles.textredcentered : styles.text}
            >
              Negativa (abono):{"\n"}
              {factura.negative.toString()}
            </Text>
          </View>
          <View>
            {factura.ticket_identifier_serie == "F1" ? (
              <Text style={styles.text}>
                Nif destinatario:{"\n"}
                {factura.nif.toString()}
              </Text>
            ) : null}
          </View>
          <View>
          ...
          <View>
            <Text style={styles.text}>
              ...
              {factura.rectificativa_por_diferencias.toString()}
            </Text>
          </View>
          <View>
            <Text style={styles.text}>
              Rectificativa sustitutiva:{"\n"}
              {factura.rectificativa_sustitutiva.toString()}
            </Text>
          </View>
          <View>
            <Text style={styles.text}>
              Comentario rectificacion:{"\n"}
              {factura.comentario_rectificacion}
            </Text>
          </View>
          <View>
            <Text style={styles.text}>
              Rectificada:{"\n"}
              {factura.rectificada?.ticket_identifier}
            </Text>
          </View>
          <View>
            {factura.ticket_identifier_serie == "F1" ? (
              <Text style={styles.text}>
                ...
                {factura.ordinaria_de_simplificada.toString()}
              </Text>
            ) : null}
          </View>
          <View>
            <Text style={styles.text}>Pedido:</Text>
            {factura.order_elements.length > 0 ? (
              factura.order_elements.map((element, index) => (
                <Text
                  style={factura.negative ? styles.textred : null}
                  key={index}
                >
                  {element.dish} (x{element.quantity}) [ID:{element.dish_pk}] -{" "}
                  {element.new_price_corrected
                    ? element.new_price_corrected
                    : element.dish_price}{" "}
                  €
                </Text>
              ...
              <Text style={factura.negative ? styles.textred : null}>
                No hay platos
              </Text>
            )}
          </View>
          <View>
            ...
            {factura.conceptos_extra.length > 0 ? (
              factura.conceptos_extra.map((element, index) => (
                <Text
                  style={factura.negative ? styles.textred : null}
                  key={index}
                >
          ...
          {factura.available_from_simplified_to_ordinary ? (
            <TouchableOpacity
              ...
              onPress={() => {
                navigation.navigate("De simplificada a ordinaria", {
                  eventId: factura.id,
                });
              }}
            >
          ...
          >
            <Text style={styles.textbutton}>
              Imprimir última factura asociada
            </Text>
          </TouchableOpacity>
        </View>
      </View>
