...
function Recompensas() {
  ...
  var fetchRecompensasNoGanadas = async (restaurantPk) => {
    const res = await fetch(
      BASE_URL + "rewards-not-gained-dc/" + restaurantPk.toString() + "/",
      {
        method: "GET",
        mode: "no-cors",
        headers: {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", async () => {
      ...
      gettingPoints(restaurantChosen_pk);
      fetchRecompensasNoGanadas(restaurantChosen_pk);
    });
    return unsubscribe;
  }, [navigation]);
  ...
  return (
    <View style={styles.container}>
      <View>
        <View><Text style={styles.pointstext}>Estás viendo las recompensas del restaurante o restaurantes {restaurantChosen.franchise}.</Text></View>
        ...
        <Text style={styles.pointstext}>
          {" "}
          Cada euro gastado en el restaurante otorga {pointsPerEuro} puntos.
        </Text>
      </View>
      <ScrollView>
        <View>
          {notAbleToFetch ? (
            ...
            <Text style={styles.pointstext}> No hay recompensas aún</Text>
          ) : (
            <View
            ...
            >
              {recompensas.map((recompensa, index) => {
                ...
                return (
                  <View
                    key={recompensa.number_of_reward}
                    style={{
                      flexBasis: `33.33%`,
                      flexGrow: 0,
                      marginBottom: 10,
                  ...
                  >
                    <View key={recompensa.number_of_reward} style={styles.card}>
                      {recompensa.hasOwnProperty(
                        "image_link_before_delivered"
                      ) ? (
                        <ScaledImage
                          uri={recompensa.image_link_before_delivered}
                          finalwidth={150}
                          style={{ marginVertical: 15 }}
                        />
                      ) : null}
                      {recompensa.hasOwnProperty("image_link") ? (
                        <ScaledImage
                          uri={recompensa.image_link}
                          finalwidth={150}
                          style={{ marginVertical: 15 }}
                        />
                      ) : null}
                      ...
                      <Text style={styles.text}>
                        {recompensa.reward_description_before_delivered ===
                        undefined
                          ? recompensa.reward_description
                          : recompensa.reward_description_before_delivered}
                      </Text>
                      {!recompensa.hasOwnProperty("code") ? (
                        <Text style={styles.text}>
                          Puntos: {recompensa.points_price}
                        </Text>
                      ) : null}

                      {!recompensa.hasOwnProperty(
                        "code"
                      ) ? null : recompensa.used ? (
                        <Text style={styles.textnotactive}>
                          {" "}
                          CODIGO: {recompensa.code} (CANJEADA)
                        </Text>
                      ) : (
                        <Text style={styles.textactive}>
                          {" "}
                          CODIGO: {recompensa.code}
                        </Text>
                      )}
                      {!recompensa.hasOwnProperty("code") ? (
                        <TouchableOpacity
                          style={styles.button}
                          onPress={async () => {
                            await editReward(recompensa.pk);
                          }}
                        >
