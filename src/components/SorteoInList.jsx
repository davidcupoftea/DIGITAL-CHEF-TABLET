...
const SorteoInList = ({ offer, isstatus, fetchSorteos }) => {
  ...
  const askBeforeDeleting = (offer_id) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Sí",
          onPress: async () => await deleteGiveaway(offer_id),
        },
      ]
    );
  };

  const deleteGiveaway = async (offer_id) => {
    Alert.alert('Borrando...', 'El elemento seleccionado se está borrando')
    const res = await fetch(
      BASE_URL + "delete-giveaway/" + restaurantChosen.pk + "/",
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ giveaway: offer_id }),
      }
    );

    var jsonData2 = await res.json();
    if (jsonData2.status == 'ok'){
    fetchSorteos(restaurantChosen.pk)
    }
  };
    ...
    <View style={offer.visible? styles.greencard : styles.redcard}>
      ...
      <View style={styles.insidecard}>
        ...
        <TouchableOpacity
          style={styles.button}
          onPress={() => {
            navigation.navigate("Sorteo", { eventId: offer.id });
          }}
        >
          <Text style={styles.textbutton}>Ver sorteo</Text>
        </TouchableOpacity>
        <TouchableOpacity
...
export default memo(SorteoInList);
