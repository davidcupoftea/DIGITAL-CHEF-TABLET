...
import NotificacionPushInList from "./NotificacionPushInList.jsx"
import getAndSetRestaurant from "../services/apiCallFavouriteRestaurant.jsx";

const NotificacionesPush = ({ route}) => {
  ...
  const fetchEmails = async (restaurantChosen_pk) => {
    ...
    const res = await fetch(
      BASE_URL + "ver-push-digital-chef/" + restaurantChosen_pk + "/",
      {
        method: "POST",
        mode: "no-cors",
        headers: {
      ...
      <ScrollView>
        ...
        {loading && !gotten ? (
          ...
          <Text style={styles.textsmall}>No puedes acceder a estos datos</Text>
            ...
            <NotificacionPushInList key={index} email={email}></NotificacionPushInList>
            ))
        )}

        {loaded && !loading && emails.length == 0 ? <Text style={styles.textsmall}>No hay notificaciones</Text>:null}
      </ScrollView>
            <TouchableOpacity
        style={styles.reservation}
        onPress={() => {
          navigation.navigate("Creación de notificación");
        }}
      >
        <Text style={styles.reservationtext}>Crear notificación</Text>
      </TouchableOpacity>
    </View>
  );
};
