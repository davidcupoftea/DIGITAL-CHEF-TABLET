...
const TrabajosScreen = (variable) => {
  ...
  const updateJob = async (restaurantChosen_pk) => {
    setDisabled(true);
    if (
      !title ||
      !description ||
    ...
    data.append("pk_of_job", variable.route.params.eventId.toString());
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false);
      Alert.alert("Éxito", "Se ha actualizado el trabajo");
      navigation.navigate('Trabajos', { refresh: true })
    }
  };

  const fetchJobs = async (restaurantChosen_pk) => {
    const res4 = await fetch(
      BASE_URL +
        "trabajos-digital-chef/" +
        restaurantChosen_pk +
        "/" +
        variable.route.params.eventId.toString() +
        "/",
