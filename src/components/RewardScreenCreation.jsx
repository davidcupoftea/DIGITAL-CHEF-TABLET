...
const RewardScreenCreation = (variable) => {

  const [reward, setReward] = useState(null);
  const [numberOfReward, setNumberOfReward] = useState('');
  const [rewardDescription, setRewardDescription] = useState(null);
  const [rewardDescriptionBeforeDelivered, setRewardDescriptionBeforeDelivered] = useState(null);
  ...
  const updateReward = async (restaurantChosen_pk) => {
    ...
    if (!reward || !numberOfReward || !rewardDescription || !rewardDescriptionBeforeDelivered || !imageLink || !pointsPrice ){
      setDisabled(false)
      Alert.alert('¡Tienes que rellenar todos los campos!','(Incluido lo imagen)')
      return
    }
    ...
    data.append("reward_description_before_delivered", rewardDescriptionBeforeDelivered);
    data.append("image_ink", imageLink);
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false)
      Alert.alert("Éxito", "Se ha creado la recompensa");
      navigation.goBack();
...
<>
        <ScrollView>
          <View style={styles.viewable}>
            ...
            <View>
              <Text style={styles.text}>Número de la recompensa (por ejemplo: 1, 2, 3...)</Text>
              <TextInput
                style={styles.textinput}
            ...
            <View>
              <Text style={styles.text}>Descripción (que se muestre antes de revelar la recompensa) </Text>
              <TextInput
                style={styles.textinput}
                multiline={true}
                numberOfLines={4}
            ...
            <View>
              <Text style={styles.text}>Puntos a alcanzar para conseguir la recompensa</Text>
              <TextInput
...
export default RewardScreenCreation;
