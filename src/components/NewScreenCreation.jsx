...
const NewsScreen = () => {
  ...
  const createNew = async (restaurantChosen_pk) => {
    setDisabled(true)
    ...
    const res4 = await fetch(
      BASE_URL + "crear-novedad-digital-chef/" + restaurantChosen_pk + "/",
      {
        method: "POST",
        headers: {
    ...
    if (jsonData4.status == "ok") {
      setDisabled(false)
      Alert.alert("Éxito", "Se ha creado la noticia");
      navigation.goBack();
